...
const EditUnitDrawer: React.FC<AddUnitDrawerProps> = ({
  isOpen,
  toggleDrawer,
  data,
}) => {
  const [input, setInput] = useState(data.name);
  const [selectedValue, setSelectedValue] = useState(data.numberType);
  ...
  const editUnit = () => {
    if (input == "") {
      ...
      ApiManager.addUnit(body).then((resp) => {
    ...
    >
      <div style={styles.drawerContainer}>
        <div style={styles.drawerHeader}>
          <label style={styles.drawerTitle}>Edit Unit</label>
